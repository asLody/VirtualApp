...
public class VProcessService extends IProcessManager.Stub {
	...
	private final List<String>  mReadyProcssList=new ArrayList<String>();              //临时保存即将开启的插件进程名称
	...
	public void killAllApps() {
		...
		mReadyProcssList.clear();          //清空插件进程名称
				...
				public void binderDied() {
					synchronized (mProcessList) {
						...
						if (r != null) {
							...
							mReadyProcssList.remove(r.processName);
						}
	...
	public ProcessRecord startProcessLocked(@NonNull ComponentInfo componentInfo) {
		...
		synchronized (this) {
			...
			if (stubInfo == null) {
				...
				notCreated = true;
				stubInfo = fetchFreeStubInfo(VActivityService.getService().getStubs(),appProcessName );
			}
		}
		if (stubInfo == null) {
	...
	private boolean isReadyProcess(String processName){
		for(String  process: mReadyProcssList ){
			if (TextUtils.equals(process, processName)) {
				return true;
			}
		}
		return false;
	...
	public synchronized StubInfo fetchFreeStubInfo(Collection<StubInfo> stubInfos,String processName) {
		for (StubInfo stubInfo : stubInfos) {
			if (!isStubProcessRunning(stubInfo) && !isReadyProcess(processName)) {
				mReadyProcssList.add(processName);
				return stubInfo;
			}
		}
		return null;
