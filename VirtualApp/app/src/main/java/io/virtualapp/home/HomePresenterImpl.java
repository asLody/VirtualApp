...
class HomePresenterImpl implements HomeContract.HomePresenter {
    ...
    public void dataChanged() {
		...
		mRepo.getVirtualApps().done(new DoneCallback<List<AppData>>() {

				@Override
				public void onDone(List<AppData> p1) {
					mView.loadFinish(p1);
				}
				...
				@Override
				public void onFail(Throwable p1) {
					mView.loadError(p1);
				}
    ...
    @Override
    public void addApp(final AppInfoLite info) {
        class AddResult {
            private PackageAppData appData;
            private int userId;
            private boolean justEnableHidden;
        }
        final AddResult addResult = new AddResult();
    ...
    private void handleOptApp(final AppData data, final String packageName, final boolean needOpt) {
        VUiKit.defer().when(new Runnable() {
				@Override
				public void run() {
					{
